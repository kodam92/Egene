...
<%
    ...
    if (!"".equals(fileConfigObj)) {
        fileMaxSize = fileConfigObj.getString("file_maxsize");
        file_ext_disable = fileConfigObj.getString("file_ext_disable");
    }
    Box box = HttpUtility.getBox(request);

    String pms_id = box.get("pms_id");
    String app_id = box.get("app_id");
    String pms_app_id = box.get("pms_app_id");
    ...
    // 현재로딩되고 있는 Projec Workspace 보안키 생성한다.
    // 보안키는 프로젝트 아이디 로그인한 사용자 아이디로 생성한다.
    String secKey = "";
    String encValue = "";

    if (_user != null) {
        secKey = _user.emp_id + "|" + pms_id;
        ISecure sec = SecureUtil.getInstance().getSecure(SecureUtil.AES);
        encValue = sec.encrypt(secKey);
...
<head>
    ...
    <!-- dhxKanban -->
    <link rel="stylesheet" href="/xplugin/kanban/kanban.css">
    ...
    <link rel="stylesheet" href="/css/kanban/wx-icons.css">
    <!-- new pms js -->
    <script src="/script/egene/pms.js"></script>

</head>

<body>

<div class="egene-root pms" pms-id="<%=pms_id%>" app_id="<%= app_id %>" pms_app_id="<%= pms_app_id %>"
     sec="<%=encValue%>" style="height: 100%;">
...
<script>
    //로그인 상태값에 따라서 Application Main or  로그인 화면을 로딩한다.
    $(document).ready(function () {

        $egene.app.id = 'pms';
        $egene.app.name = 'pms';
        $egene.app.title = '<%=HttpUtility.translate(title)%>';
        $egene.app.commentSQL = 'Mention.Lookup.PMS';
